...
function Get-AvailableStorageAccountName {
    ...
    $graphQueryResult = Search-AzGraph -Query $sAGraphQuery -First 1000 -Verbose
    #check storage accounts exists in the subscription
    ...
    foreach($responses in $response){
      if ($responses.value.name -eq $storageAccountName ) {
    Write-Host "The storage account $storageAccountName already exists"
    }#check storage accounts exists in the subscription
